...
object NavUtil {
    ...
    /*
    check bottom indicator, MIUI>12
     */
    fun hasBottomIndicator(context: Context) = when {
        RomUtil.checkIsMiuiRom()
                && isMiuiFullScreen(context)
                && RomUtil.getMiuiVersion() >= 12
                && getNavigationBarHeight(context) <= 44 -> true
        else -> false
    }
    ...
    private fun isHasNavigationBar(context: Context): Boolean {
        ...
        if (displayHeight > displayWidth) {
            if (displayHeight + DisplayUtil.getNavigationBarHeight(context) > realHeight) return false
        } else {
            if (displayWidth + DisplayUtil.getNavigationBarHeight(context) > realWidth) return false
        }
